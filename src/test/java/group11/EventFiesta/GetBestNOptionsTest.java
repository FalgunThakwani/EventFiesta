...
public class GetBestNOptionsTest {
    ...
    // List<GroupComponent> bestFiveServiceProviders = new
    // ArrayList<>(serviceProviders);
    // if (serviceProviders.size() > n) {
    // bestFiveServiceProviders =
    // serviceProviders.stream().limit(n).collect(Collectors.toList());
    // }
    ...
    // getBestNOptions.getAllGroups(servicesAndProviders, allGroups, new
    // OrganizerGroup());
    // List<GroupComponent> bestNGroups = getBestNOptions.getBestNGroups(allGroups,
    // n);
